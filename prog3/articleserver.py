    ...
    with db_cursor() as cur:
        ...
        for article in articles:
            ...
            cur.execute('SELECT author_name'
                        ...
                        ' WHERE article_id = %s'
                        ' ORDER BY position', (article['id'],))
            authors = []

            #adds authors from paper
            for name, in cur:
                authors.append(name + ',')
...
def authorLeaders():
    with db_cursor() as cur:
        ...
        authors = [{'name': '<a href="/authors/'+str(id)+'">'+name+'</a>', 'totalArticles': total}
              for (id, name, total) in cur]
    return flask.render_template('leaders.html', authors=authors)
